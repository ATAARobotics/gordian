...
final class If extends Scope {
    ...
    public void run(String script) throws Exception {
        ...
        if (Strings.contains(script, "if(")) {
            ...
            run.add(script.substring(script.indexOf(");") + 2, script.indexOf("else") >= 0 ? script.indexOf("else") : script.length()));
            script = script.substring(script.indexOf("else") >= 0 ? script.indexOf("else") : script.length());
        }
        while (Strings.contains(script, "elseif(")) {
            cases.add(script.substring(script.indexOf("elseif(") + 7, script.indexOf(");")));
            run.add(script.substring(script.indexOf(");") + 2, script.substring(6).indexOf("else") >= 0 ? script.substring(6).indexOf("else") + 6 : script.length()));
            script = script.substring(script.substring(6).indexOf("else") >= 0 ? script.substring(6).indexOf("else") + 6 : script.length());
        }
